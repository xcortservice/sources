...
def vowel(value: str) -> str:
    return (
        ("an" if value[0].lower() in "aeiou" else "a")
        + " "
        + value
    )
...
def duration(value: float, ms: bool = True) -> str:
    h = (
        ...
        if ms
        else int((value / (60 * 60)) % 24)
    )
    m = (
        int((value / (1000 * 60)) % 60)
        if ms
        else int((value / 60) % 60)
    )
...
def snake_cased_dict(
    ...
    def _visit(p, k, v):
        k = snake_cased(str(k))
        if k in discard_keys or (
            remove_nulls
            and ((not v and all_nulls) or v == "")
        ):
            return False
        return (k, v)
